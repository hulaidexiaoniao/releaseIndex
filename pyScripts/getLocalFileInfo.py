...
def get_path(path, path_tree, is_need_file=False, max_level=1):
    ...
    if os.path.isdir(path):
        ...
        for current_dir in os.listdir(path):
            if current_dir != 'ep_python-3.7.3':
...
def getLocalFileInfo(base_path:str)->str:
    ...
    s = open(Name + "_checkList.txt","a+",encoding="utf-8")
    while True:
        line = f.readline()
        if line == "---\n" or line == "\n":
            continue
...
def checkList_produce(base_path1):
    global base_path   
    base_path = base_path1  
    getLocalFileInfo(base_path)
